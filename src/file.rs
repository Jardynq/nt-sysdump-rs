...
impl ImageFile {
    ...
    pub(crate) fn new(
        ...
        let src = {
            let len = std::fs::metadata(path).map_err(ImageError::IoError)?.len();
            let mut buf = Vec::with_capacity(len as usize);
            let mut file = std::fs::File::open(path).map_err(ImageError::IoError)?;
            file.read_to_end(&mut buf).map_err(ImageError::IoError)?;
            buf
        };
        ...
        for section in headers.sections {
            ...
            let src_end = src_start
                ...
                .ok_or(ImageError::InvalidSection)?;
            if src_end > src.len() {
                return Err(ImageError::InvalidSection);
            }
